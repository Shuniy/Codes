...
def ZeroOneKnapsackProblemHelper(index: int, profit: int, capacity: int, weights: list, values: list[int]) -> int:
    ...
    if weights[index] <= capacity:
        ...
        profit1 = ZeroOneKnapsackProblemHelper(index + 1, profit, capacity, weights, values)
        # Will return max profit by adding the item
        profit2 = ZeroOneKnapsackProblemHelper(index + 1, profit + values[index], capacity - weights[index], weights, values)
    else:
...
def ZeroOneKnapsackProblemMemo(capacity, weights, values) -> int:
    memo = [[-1 for _ in range(0, capacity + 1)] for _ in range(len(weights) + 1)]
