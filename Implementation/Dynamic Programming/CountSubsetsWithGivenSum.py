...
def CountSubsetsWithGivenSumRecursiveHelper(arr: list[int], targetSum: int, index: int, count: int) -> int:
    ...
    if arr[index] <= targetSum:
        count = CountSubsetsWithGivenSumRecursiveHelper(arr, targetSum - arr[index], index + 1, count)
        count = CountSubsetsWithGivenSumRecursiveHelper(arr, targetSum, index + 1, count)
    else:
        count = CountSubsetsWithGivenSumRecursiveHelper(arr, targetSum, index + 1, count)
    return count
...
def CountSubsetsWithGivenSumTopDown(arr: list[int], targetSum: int) -> int:
    dp: list[list[int]] = [[0 for _ in range(targetSum + 1)] for _ in range(len(arr) + 1)]
