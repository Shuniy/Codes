...
class Node(object):
    ...
    def fusion_nodes(node_1, node_2):
        ...
        def __repr__(self):
            return "Node of characters : {} | frequency : {}".format(self.char, self.freq)


class Queue(object):
    def __init__(self, string):
        _ = collections.Counter(string)
        self.arr = [Node(char=letter, freq=[letter]) for letter in _]
        self.sort()
    ...
    def fuse_step(self) -> None:
        ...
        self.arr.append(Node.fusion_nodes(
...
class HuffmanEncoder(object):
    ...
    def _create_encoding_table(base_code : str, node : Node) -> list:
        ...
        if node.left is not None:
            coding_dict.append((HuffmanEncoder._create_encoding_table(current_code, node.left)))
